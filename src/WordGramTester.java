...
public class WordGramTester {
	...
	public void testShiftAdd(){
		String[] words = {"apple", "zebra", "mongoose", "hat"};
		WordGram a = new WordGram(words,0,4);
		WordGram b = new WordGram(words,0,1);
		WordGram a2 = new WordGram(words,0,3);
		WordGram b2 = new WordGram(words,2,0);
		
		assertEquals("comp fail self",a.shiftAdd("dog").toString(), "{zebra, mongoose, hat, dog}");
		assertEquals("comp fail self",b.shiftAdd("dog").toString(), "{dog}");
		assertEquals("fail sub", a2.shiftAdd("dog").toString(), "{zebra, mongoose, dog}");
		assertEquals("fail empty",b2.shiftAdd("dog").toString(), "{}");
	}
	@Test
	public void testToString(){
		...
		assertEquals("comp fail self",a.toString(), "{apple, zebra, mongoose, hat}");
		assertEquals("comp fail copy",b.toString(), "{apple, zebra, mongoose, hat}");
		assertEquals("fail sub", a2.toString(), "{apple, zebra, mongoose}");
		assertEquals("fail empty",b2.toString(), "{}");
	}
